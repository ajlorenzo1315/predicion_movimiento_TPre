...
def test_simple(args):
    ...
    with torch.no_grad():
        for idx, image_path in enumerate(paths):
            ...
            cv2.putText(colormapped_im, '%s ' % (" ".join(["DIFFNet", args.model_name]) ),
                (0, int(15 * text_scale)), cv2.FONT_HERSHEY_PLAIN, 2, (255, 255, 255), thickness=2)

            cv2.putText(colormapped_im, 'fps: %.2f' % (1. / timer.average_time),
            ...
            #name_grey_depth = os.path.join('results',"{}_grey_disp.png".format(name_foto))
            ...
            print("   Processed {:d} in fps {:f}  of {:d} images - saved prediction to {} ".format(
                idx + 1, 1. / timer.average_time ,len(paths), name_dest_im))
